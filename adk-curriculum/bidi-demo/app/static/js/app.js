...
function startVideoStream() {
  if (is_video_streaming) return;
  is_video_streaming = true;
  document.getElementById("streamVideo").innerText = "⏹️ Stop Stream";
  ...
  videoInterval = setInterval(() => {
    if (!cameraStream || !is_video_streaming) return;
    
    // 🛡️ Guard: Dimension might be 0 until full stream triggers
    if (cameraPreview.videoWidth === 0) return;

    // Scale down for bandwidth efficiency (e.g. 480px width)
    canvas.width = 480; 
    canvas.height = (cameraPreview.videoHeight / cameraPreview.videoWidth) * 480; 
    ...
    frameCount++;
    if (frameCount % 5 === 0) { // Log once per second at 5 FPS
      addConsoleEntry('outgoing', `Streaming Video Frame (${canvas.width}x${Math.round(canvas.height)})`, null, '🎥', 'user');
    }
  }, 200); // 5 FPS
}
...
function stopVideoStream() {
  ...
  const streamHeaderBtn = document.getElementById("streamVideo");
  if (streamHeaderBtn) {
    streamHeaderBtn.innerText = "▶️ Stream Video";
  }
}

// Close camera modal and stop preview
function closeCameraPreview() {
  stopVideoStream(); // ⏹️ Stop streaming if active
...
if (streamVideoBtn) {
  streamVideoBtn.addEventListener("click", () => {
    if (!is_video_streaming) startVideoStream();
